module.exports = function(num_of_routes_to_add,gym_of_routes,existing_route_numbers) {
    ...
    while (added_num < num_of_routes_to_add ) {
        ...
        if (existing_route_numbers.indexOf(route_name_count) === -1) {
            routes_array.push({route_name: 'Route ' + (route_name_count), route_number: route_name_count, gym_name: gym_of_routes, setter_input: {setter_grade: 5}, current_grade_average: 5, current_star_rating: 4 })
            added_num++
            route_name_count++
        } else {
            route_name_count++
